 ...
 *
 * - `site`: 必填，site id
 * - `endpoint`: 預設是 `http://webtracking.urad.com.tw/log`
 *
...
proto.endpoint = function () {
  return this.get('endpoint')
    || 'http://webtracking.urad.com.tw/log';
};
...
proto.track = function (event, value, cb) {
  ...
  if (!(event instanceof TrackingEvent)) {
    event = TrackingEvent.create(event, null, []);
    event.value = value;
  }
...
proto.trackEvent = function (analyticsEvent, target, targetEvent, valueGatter, cb) {
  var eventBinder = EventBinder.create(target);
